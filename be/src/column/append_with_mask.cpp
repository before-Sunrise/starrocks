...
template <bool PositiveSelect>
inline bool AppendWithMaskVisitor<PositiveSelect>::is_selected(uint8_t value) const {
...
size_t AppendWithMaskVisitor<PositiveSelect>::count_selected() const {
    ...
    if constexpr (PositiveSelect) {
        return SIMD::count_nonzero(_mask, _count);
    } else {
        size_t selected = 0;
...
Status AppendWithMaskVisitor<PositiveSelect>::append_fixed_length(FixedLengthColumnBase<T>* column) {
    ...
    auto* dest = data.data() + orig_size;
    const T* src_data = reinterpret_cast<const T*>(src_column->raw_data());
