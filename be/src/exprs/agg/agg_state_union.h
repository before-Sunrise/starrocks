...
public:
    ...
    void update(FunctionContext* ctx, const Column** columns, AggDataPtr __restrict state,
        ...
        VLOG(2) << "AggStateUnion::update, input:" << columns[0]->debug_string() << ", row_num:" << row_num;
        _function->merge(ctx, columns[0], state, row_num);
    }

    void merge(FunctionContext* ctx, const Column* column, AggDataPtr __restrict state, size_t row_num) const override {
        VLOG(2) << "AggStateUnion::merge, input:" << column->debug_string() << ", row_num:" << row_num;
    ...
    void serialize_to_column([[maybe_unused]] FunctionContext* ctx, ConstAggDataPtr __restrict state,
        ...
        VLOG(2) << "AggStateUnion::serialize_to_column, to:" << to->debug_string();
    }
    ...
    void convert_to_serialize_format([[maybe_unused]] FunctionContext* ctx, const Columns& srcs, size_t chunk_size,
        ...
        VLOG(2) << "AggStateUnion::convert_to_serialize_format, srcs[0]:" << srcs[0]->debug_string();
        *dst = srcs[0];
    }
    ...
    void finalize_to_column(FunctionContext* ctx __attribute__((unused)), ConstAggDataPtr __restrict state,
                            Column* to) const override {
        _function->serialize_to_column(ctx, state, to);
        VLOG(2) << "AggStateUnion::finalize_to_column, to:" << to->debug_string();
    }
