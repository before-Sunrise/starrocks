...
public:
    ...
    void insert_hash(const uint64_t hash) noexcept {
        ...
        auto [masks_0, masks_1] = make_mask(hash >> _log_num_buckets);

        uint32x4_t directory_1 = vld1q_u32(&_directory[bucket_idx][0]);
        uint32x4_t directory_2 = vld1q_u32(&_directory[bucket_idx][4]);

        directory_1 = vorrq_u32(directory_1, masks_0);
        directory_2 = vorrq_u32(directory_2, masks_1);
    ...
    bool test_hash(const uint64_t hash) const noexcept {
        ...
        uint32x4_t out_1 = vbicq_u32(masks_0, directory_1);
        uint32x4_t out_2 = vbicq_u32(masks_1, directory_2);
...
#ifdef __ARM_NEON
    // For Neon version:
    std::pair<uint32x4_t, uint32x4_t> make_mask(uint32_t key) const noexcept {
        ...
        return {vshlq_u32(ones, reinterpret_cast<int32x4_t>(hash_data_1)),
                vshlq_u32(ones, reinterpret_cast<int32x4_t>(hash_data_2))};
    }
#endif
