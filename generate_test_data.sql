...
-- 插入数据，实现不同过滤性
INSERT INTO test_predicate_few_char
SELECT 
    -- 谓词列：实现不同过滤性的数据分布
    CASE 
        WHEN rand_val % 1000 < 10 THEN CONCAT('rare_value_', CAST(rand_val % 10 AS STRING))      -- 1% 高过滤性
        WHEN rand_val % 1000 < 100 THEN CONCAT('medium_value_', CAST(rand_val % 10 AS STRING))   -- 9% 中等过滤性  
        ELSE CONCAT('common_value_', CAST(rand_val % 10 AS STRING))                              -- 90% 低过滤性
    END as filter_col1,
    
    CASE 
        WHEN (rand_val + 1) % 1000 < 10 THEN CONCAT('rare_value_', CAST((rand_val + 1) % 10 AS STRING))
        WHEN (rand_val + 1) % 1000 < 100 THEN CONCAT('medium_value_', CAST((rand_val + 1) % 10 AS STRING))
        ELSE CONCAT('common_value_', CAST((rand_val + 1) % 10 AS STRING))
    END as filter_col2,
    
    -- 非谓词列：随机数据
    CONCAT('data_', LPAD(CAST(rand_val % 10000 AS STRING), 45, '0')) as data_col1,
    CONCAT('data_', LPAD(CAST((rand_val + 1) % 10000 AS STRING), 45, '0')) as data_col2,
    CONCAT('data_', LPAD(CAST((rand_val + 2) % 10000 AS STRING), 45, '0')) as data_col3,
    CONCAT('data_', LPAD(CAST((rand_val + 3) % 10000 AS STRING), 45, '0')) as data_col4,
    CONCAT('data_', LPAD(CAST((rand_val + 4) % 10000 AS STRING), 45, '0')) as data_col5,
    CONCAT('data_', LPAD(CAST((rand_val + 5) % 10000 AS STRING), 45, '0')) as data_col6,
    CONCAT('data_', LPAD(CAST((rand_val + 6) % 10000 AS STRING), 45, '0')) as data_col7,
    CONCAT('data_', LPAD(CAST((rand_val + 7) % 10000 AS STRING), 45, '0')) as data_col8,
    CONCAT('data_', LPAD(CAST((rand_val + 8) % 10000 AS STRING), 45, '0')) as data_col9,
    CONCAT('data_', LPAD(CAST((rand_val + 9) % 10000 AS STRING), 45, '0')) as data_col10,
    CONCAT('data_', LPAD(CAST((rand_val + 10) % 10000 AS STRING), 45, '0')) as data_col11,
    CONCAT('data_', LPAD(CAST((rand_val + 11) % 10000 AS STRING), 45, '0')) as data_col12,
    CONCAT('data_', LPAD(CAST((rand_val + 12) % 10000 AS STRING), 45, '0')) as data_col13,
    CONCAT('data_', LPAD(CAST((rand_val + 13) % 10000 AS STRING), 45, '0')) as data_col14,
    CONCAT('data_', LPAD(CAST((rand_val + 14) % 10000 AS STRING), 45, '0')) as data_col15,
    CONCAT('data_', LPAD(CAST((rand_val + 15) % 10000 AS STRING), 45, '0')) as data_col16,
    CONCAT('data_', LPAD(CAST((rand_val + 16) % 10000 AS STRING), 45, '0')) as data_col17,
    CONCAT('data_', LPAD(CAST((rand_val + 17) % 10000 AS STRING), 45, '0')) as data_col18,
    CONCAT('data_', LPAD(CAST((rand_val + 18) % 10000 AS STRING), 45, '0')) as data_col19,
    CONCAT('data_', LPAD(CAST((rand_val + 19) % 10000 AS STRING), 45, '0')) as data_col20
FROM data_generator;
...
INSERT INTO test_predicate_many_char
SELECT 
    -- 10个谓词列
    CASE 
        WHEN rand_val % 1000 < 10 THEN CONCAT('rare_value_', CAST(rand_val % 10 AS STRING))
        WHEN rand_val % 1000 < 100 THEN CONCAT('medium_value_', CAST(rand_val % 10 AS STRING))
        ELSE CONCAT('common_value_', CAST(rand_val % 10 AS STRING))
    END as filter_col1,
    ...
    CASE 
        WHEN (rand_val + 2) % 1000 < 10 THEN CONCAT('rare_value_', CAST((rand_val + 2) % 10 AS STRING))
        WHEN (rand_val + 2) % 1000 < 100 THEN CONCAT('medium_value_', CAST((rand_val + 2) % 10 AS STRING))
        ELSE CONCAT('common_value_', CAST((rand_val + 2) % 10 AS STRING))
    END as filter_col3,
    
    CASE 
        WHEN (rand_val + 3) % 1000 < 10 THEN CONCAT('rare_value_', CAST((rand_val + 3) % 10 AS STRING))
        WHEN (rand_val + 3) % 1000 < 100 THEN CONCAT('medium_value_', CAST((rand_val + 3) % 10 AS STRING))
        ELSE CONCAT('common_value_', CAST((rand_val + 3) % 10 AS STRING))
    END as filter_col4,
    
    CASE 
        WHEN (rand_val + 4) % 1000 < 10 THEN CONCAT('rare_value_', CAST((rand_val + 4) % 10 AS STRING))
        WHEN (rand_val + 4) % 1000 < 100 THEN CONCAT('medium_value_', CAST((rand_val + 4) % 10 AS STRING))
        ELSE CONCAT('common_value_', CAST((rand_val + 4) % 10 AS STRING))
    END as filter_col5,
    
    CASE 
        WHEN (rand_val + 5) % 1000 < 10 THEN CONCAT('rare_value_', CAST((rand_val + 5) % 10 AS STRING))
        WHEN (rand_val + 5) % 1000 < 100 THEN CONCAT('medium_value_', CAST((rand_val + 5) % 10 AS STRING))
        ELSE CONCAT('common_value_', CAST((rand_val + 5) % 10 AS STRING))
    END as filter_col6,
    
    CASE 
        WHEN (rand_val + 6) % 1000 < 10 THEN CONCAT('rare_value_', CAST((rand_val + 6) % 10 AS STRING))
        WHEN (rand_val + 6) % 1000 < 100 THEN CONCAT('medium_value_', CAST((rand_val + 6) % 10 AS STRING))
        ELSE CONCAT('common_value_', CAST((rand_val + 6) % 10 AS STRING))
    END as filter_col7,
    
    CASE 
        WHEN (rand_val + 7) % 1000 < 10 THEN CONCAT('rare_value_', CAST((rand_val + 7) % 10 AS STRING))
        WHEN (rand_val + 7) % 1000 < 100 THEN CONCAT('medium_value_', CAST((rand_val + 7) % 10 AS STRING))
        ELSE CONCAT('common_value_', CAST((rand_val + 7) % 10 AS STRING))
    END as filter_col8,
    
    CASE 
        WHEN (rand_val + 8) % 1000 < 10 THEN CONCAT('rare_value_', CAST((rand_val + 8) % 10 AS STRING))
        WHEN (rand_val + 8) % 1000 < 100 THEN CONCAT('medium_value_', CAST((rand_val + 8) % 10 AS STRING))
        ELSE CONCAT('common_value_', CAST((rand_val + 8) % 10 AS STRING))
    END as filter_col9,
    
    CASE 
        WHEN (rand_val + 9) % 1000 < 10 THEN CONCAT('rare_value_', CAST((rand_val + 9) % 10 AS STRING))
        WHEN (rand_val + 9) % 1000 < 100 THEN CONCAT('medium_value_', CAST((rand_val + 9) % 10 AS STRING))
        ELSE CONCAT('common_value_', CAST((rand_val + 9) % 10 AS STRING))
    END as filter_col10,
...
-- 显示表的行数
SELECT 'test_predicate_few_char' as table_name, COUNT(*) as row_count FROM test_predicate_few_char
UNION ALL
SELECT 'test_predicate_few_varchar' as table_name, COUNT(*) as row_count FROM test_predicate_few_varchar
UNION ALL
SELECT 'test_predicate_many_char' as table_name, COUNT(*) as row_count FROM test_predicate_many_char
UNION ALL
